...
CourseListModel::CourseListModel(QObject* parent) : QAbstractListModel{parent} {
  ...
  connect(manager, &Network::NetworkManager::responseToRequest, this,
          &CourseListModel::onReply);

  connect(manager, &Network::NetworkManager::requestFailed,
          [this](QString error) { qInfo() << error; });
}
...
void CourseListModel::onReply(ReplyTypes::Reply reply) {
  beginResetModel();
  std::visit(util::match{[this](const ReplyTypes::Type::Course& course) {
                           const auto data = course.course;
                           m_courses.clear();
                           beginInsertRows(QModelIndex(), 0, data.size() - 1);
                           for (int i = 0; i < data.size(); ++i) {
                             Course cc = {data.at(i)["name"].toString(),
                                          data.at(i)["id"].toString(), i};
