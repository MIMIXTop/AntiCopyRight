...
{
    connect(nm, &Network::NetworkManager::responseToRequest,
            this, &CoursWorksListModel::onReply, Qt::QueuedConnection);
    connect(nm, &Network::NetworkManager::requestFailed,
            this, [](const QString err){ qWarning() << "Works request failed:" << err;});
}
...
void CoursWorksListModel::onReply(ReplyTypes::Reply reply)
{
    ...
    std::visit( util::match{
                   [this]( ReplyTypes::Type::CourseWorks courseWorks){
