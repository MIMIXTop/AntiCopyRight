...
TEST_F(Doc2ModelTesting, EmptiVectors) {
  EXPECT_THROW(Model::Doc2VecModel::Similarity(vec1, empty),
               std::runtime_error);
  EXPECT_THROW(Model::Doc2VecModel::Similarity(empty, vec1),
               std::runtime_error);
  EXPECT_THROW(Model::Doc2VecModel::Similarity(empty, empty),
               std::runtime_error);
}