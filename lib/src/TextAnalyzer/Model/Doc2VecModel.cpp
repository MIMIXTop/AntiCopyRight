...
Model::Doc2VecModel::getDocVector(const std::string& document) const {
  ...
  at::Tensor input =
      torch::from_blob(indices.data(), indices.size(), torch::TensorOptions())
          .to(device);
...
void Model::Doc2VecModel::loadModel() {
  ...
  word_embeddings = std::make_unique<torch::jit::Module>(
      model->attr("word_embed").toModule());
...
torch::Device Model::Doc2VecModel::setDevice() {
  return torch::Device(torch::cuda::is_available() ? torch::kCUDA
                                                   : torch::kCPU);
}
...
std::unordered_map<std::string, int32_t> Model::Doc2VecModel::setWord2Idx() {
  ...
  if (std::ifstream file(PATH_WORD2IDX); file.is_open()) {
    ...
    throw std::runtime_error("Could not open file to path: " +
                             std::string(PATH_WORD2IDX));
  }
