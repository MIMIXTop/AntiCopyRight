  ...
  asio::co_spawn(
      ...
      [this, handler = std::move(func)] -> asio::awaitable<void> {
        http::response<http::dynamic_body> res =
            co_await classroomSession_->sendRequest(
                requestHandler(DTOCourseList{}));
...
void ClassroomManager::getListCoursesWorks(HandlerFunction func,
...
void ClassroomManager::getStudentsList(HandlerFunction func,
                                       const std::string &courseId) {
  asio::co_spawn(
      ioContext_,
      [this, handler = std::move(func), courseId] -> asio::awaitable<void> {
...
void ClassroomManager::downloadStudentWork(HandlerFunction func,
...
ClassroomManager::requestHandler(DTOCreateRequest &&dto) {
  ...
  std::visit(
      util::match{[&request]([[maybe_unused]] DTOCourseList) {
                  ...
                  [&request](DTOCourseWorksList courseWorksList) {
                    request.target(std::format("/v1/courses/{}/courseWork",
                                               courseWorksList.courseId));
                    request.set(http::field::accept, "application/json");
                    request.set(http::field::host, GOOGLE_CLASSROOM_HOST);
                  },
                  [&request](DTOStudentsList studentsList) {
                    request.target(std::format("/v1/courses/{}/students",
