...
asio::awaitable<void> Session::connectToSender(const std::string& host) {
  auto endpoint =
      co_await resolver_.async_resolve(host, "443", asio::use_awaitable);
  co_await asio::async_connect(stream_.next_layer(), endpoint,
                               asio::use_awaitable);
...
asio::awaitable<http::response<http::dynamic_body>>
Session::sendRequest(http::request<http::empty_body> req) {
