...
bool AuthenticationManager::EmptyAccessToken() const { return config_.accessToken.empty(); }

bool AuthenticationManager::EmptyRefreshToken() { return config_.refreshToken.empty(); }
...
void AuthenticationManager::handleGetTokens(std::string_view boby) {
    ...
    timer.async_wait(std::bind(&AuthenticationManager::updateTokens, this, std::placeholders::_1));
}
...
void AuthenticationManager::handleRequest(http::request<http::string_body>& req, RequestStatus status) {
    ...
    switch (status) {
        case RequestStatus::GET_TOKEN:
            ...
            req.body() =
                "code=" + config_.code + "&" + "client_id=" + config_.clientId + "&" +
                "client_secret=" + config_.clientSecret + "&" + "redirect_uri=http://127.0.0.1:8080/code&" +
        ...
        case RequestStatus::UPDATE_TOKENS:
            ...
            req.set(http::field::accept, "application/json");
            req.body() =
                "client_id=" + config_.clientId + "&" + "client_secret=" + config_.clientSecret + "&" +
...
std::string_view AuthenticationManager::getToken() const { return config_.accessToken; }
